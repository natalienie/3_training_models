...
def train_test_split(feature, lable, test_potion=0.2):
    '''
...
def find_best_model(X_train, y_train, X_test, y_test):
    SVM_score = Apply_SVM(X_train, y_train, X_test, y_test)
    print('Accuracy of SVM model is: {}'.format(SVM_score))
    Gaussian_score = Apply_GaussianNB(X_train, y_train, X_test, y_test)
    print('Accuracy of Gaussian model is: {}'.format(Gaussian_score)
    Forest_score = Apply_RandomForest(X_train, y_train, X_test, y_test)
    print('Accuracy of RandomForest model is: {}'.format(Forest_score)
